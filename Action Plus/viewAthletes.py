...
def view_athletes(participant_name=None):
    ...
    print("-" * 100)
    print(f"{'Name':<25} {'Date':<15} {'Type':<15} {'Movements':<12} {'Avg Score':<12} {'Sessions':<10} {'Last Session':<20}")
    print("-" * 100)
    
    for row in results:
        name, date, mtype, num_movements, avg_score, sessions, last_session = row
        avg_str = f"{avg_score:.2f}" if avg_score else "N/A"
        last_str = last_session[:19] if last_session else "N/A"
        print(f"{name:<25} {date:<15} {mtype:<15} {num_movements:<12} {avg_str:<12} {sessions:<10} {last_str:<20}")
    
    print("-" * 100)
    
    # Summary statistics
    if participant_name:
        total_movements = sum(r[3] for r in results)
        ...
        print(f"  Movement types: {', '.join(set(r[2] for r in results))}")
    else:
